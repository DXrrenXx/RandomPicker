...
public static class Store
{
    ...
    public static void Load()
    {
        ...
        // 防御手工编辑过的 data.json：JSON 中的 null 会覆盖属性初始化值，导致后续空引用
        Data.Classes ??= new List<ClassInfo>();
    ...
    {
        // 先写临时文件再替换，避免写入中途断电 / 崩溃损坏 data.json
        var tmp = FilePath + ".tmp";
        File.WriteAllText(tmp, JsonSerializer.Serialize(Data, JsonOptions), new UTF8Encoding(true));
        if (File.Exists(FilePath))
