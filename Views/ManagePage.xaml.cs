...
public sealed partial class ManagePage : Page
{
    ...
    {
        // 优先选中当前正在点名的班级（由 MainWindow 传入）
        RebuildClassList(e.Parameter as Guid?);
    }
    ...
    /// <param name="selectId">优先选中的班级；为空时保持当前选中，选中项已不存在则回退到第一个。</param>
    private void RebuildClassList(Guid? selectId = null)
    {
        var prevId = selectId ?? SelectedClass?.Id;

        ClassList.ItemsSource = Store.Data.Classes.ToList();

        var idx = prevId is Guid id ? Store.Data.Classes.FindIndex(c => c.Id == id) : -1;
        ClassList.SelectedIndex = idx >= 0 ? idx : (Store.Data.Classes.Count > 0 ? 0 : -1);
    ...
    private async void ImportTxt_Click(object sender, RoutedEventArgs e)
    {
        ...
        var skipped = names.Count - imported;
        ShowFeedback("导入成功",
            skipped > 0
                ? $"已导入 {imported} 个名字到「{target.Name}」，{skipped} 个重复名字已跳过。"
                : $"已导入 {imported} 个名字到「{target.Name}」。",
    ...
    private async void RenameStudent_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (name != old && cls.Students.Contains(name))
        {
            ShowFeedback("未重命名", $"名单中已存在「{name}」，请换一个名字。", InfoBarSeverity.Warning);
            return;
        }
